...
bool SingleAgentSpaceTimeState::isEqual(const SingleAgentSpaceTimeState &other) const{
    return position == other.position && timestep == other.timestep;
}

int SingleAgentSpaceTimeState::getPosition() const {
    return position;
}

int SingleAgentSpaceTimeState::getTimestep() const {
    return timestep;
}
