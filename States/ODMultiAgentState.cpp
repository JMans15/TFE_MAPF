...
ODMultiAgentState::ODMultiAgentState(const std::vector<int> &positions,
                                     std::vector<int> prePositions_,
                                     int agentToAssign, bool standard,
                                     const std::vector<u_int8_t> &cannotMove_)
    : positions(positions), agentToAssign(agentToAssign), standard(standard),
...
std::vector<int> ODMultiAgentState::getPositions() const { return positions; }

std::vector<int> ODMultiAgentState::getPrePositions() const {
  return prePositions;
}

int ODMultiAgentState::getAgentToAssign() const { return agentToAssign; }

bool ODMultiAgentState::isStandard() const { return standard; }
