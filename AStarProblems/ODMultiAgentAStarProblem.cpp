...
ODMultiAgentAStarProblem::ODMultiAgentAStarProblem(
    std::shared_ptr<MultiAgentProblem> problem)
    : AStarProblem<ODMultiAgentState>(), problem(std::move(problem)) {}

std::shared_ptr<ODMultiAgentState>
ODMultiAgentAStarProblem::getStartState() const {
  return std::make_shared<ODMultiAgentState>(problem->getStarts(),
                                             problem->getStarts(), 0, true);
}

bool ODMultiAgentAStarProblem::isGoalState(
    std::shared_ptr<ODMultiAgentState> state) const {
...
bool ODMultiAgentAStarProblem::notAlreadyOccupiedPosition(
...
bool ODMultiAgentAStarProblem::notAlreadyOccupiedEdge(
...
ODMultiAgentAStarProblem::getSuccessors(
    std::shared_ptr<ODMultiAgentState> state) const {
  std::vector<std::tuple<std::shared_ptr<ODMultiAgentState>, int, int>>
      successors;
  ...
  if (problem->getObjFunction() != SumOfCosts) {
    ...
    for (int j : problem->getGraph()->getNeighbors(positions[agentToAssign])) {
      ...
      if (notAlreadyOccupiedPosition(j, positions, agentToAssign) &&
        ...
        auto successor = std::make_shared<ODMultiAgentState>(
            newpositions, prePositions, nextAgentToAssign, isStandard);
    ...
    if (notAlreadyOccupiedPosition(positions[agentToAssign], positions,
                                   agentToAssign)) {
      auto successor = std::make_shared<ODMultiAgentState>(
          positions, prePositions, nextAgentToAssign, isStandard);
    ...
    if (state->canMove(agentToAssign)) { // agentToAssign is allowed to move
      ...
      if (notAlreadyOccupiedPosition(positions[agentToAssign], positions,
                                     agentToAssign)) {
        auto successor = std::make_shared<ODMultiAgentState>(
            positions, prePositions, nextAgentToAssign, isStandard, cannotMove);
        successors.emplace_back(successor, costWait, 0);
      }
    }
  }
...
ODMultiAgentAStarProblem::getPositions(
    std::vector<std::shared_ptr<ODMultiAgentState>> states) const {
