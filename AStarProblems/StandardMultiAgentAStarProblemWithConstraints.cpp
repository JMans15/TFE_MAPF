...
StandardMultiAgentAStarProblemWithConstraints::StandardMultiAgentAStarProblemWithConstraints(std::shared_ptr<MultiAgentProblem>  problem)
        : AStarProblem<StandardMultiAgentSpaceTimeState>()
        , problem(std::move(problem))
{}

std::shared_ptr<StandardMultiAgentSpaceTimeState> StandardMultiAgentAStarProblemWithConstraints::getStartState() const {
    return std::make_shared<StandardMultiAgentSpaceTimeState>(problem->getStarts(), problem->getStartTime());
}

bool StandardMultiAgentAStarProblemWithConstraints::isGoalState(std::shared_ptr<StandardMultiAgentSpaceTimeState> state) const {
...
bool StandardMultiAgentAStarProblemWithConstraints::notAlreadyOccupiedPosition(int position, std::vector<int> &positions, int agentToAssign) const {
...
bool StandardMultiAgentAStarProblemWithConstraints::notAlreadyOccupiedEdge(int position, const std::vector<int> &positions, int agentToAssign, const std::vector<int> &prePositions) const {
    for (int i = 0; i < agentToAssign; i++) {
    ...
    if (problem->getObjFunction() != SumOfCosts){
        if (agentToAssign==problem->getNumberOfAgents()-1){
            ...
            for (int j : problem->getGraph()->getNeighbors(positions[agentToAssign])) {
                ...
                if (notAlreadyOccupiedPosition(j, positions, agentToAssign) && notAlreadyOccupiedEdge(j, positions, agentToAssign, prePositions) && problem->okForConstraints(agentToAssign, positions[agentToAssign], j, t+1)) {
                    auto successor = std::make_shared<StandardMultiAgentSpaceTimeState>(newpositions, t+1);
                    successors->emplace_back(successor, cost+costMovement, violations+problem->numberOfViolations(agentToAssign, positions[agentToAssign], j, t+1));
                }
            }
            // Wait
            if (notAlreadyOccupiedPosition(positions[agentToAssign], positions, agentToAssign) && problem->okForConstraints(agentToAssign, positions[agentToAssign], t+1)) {
                auto successor = std::make_shared<StandardMultiAgentSpaceTimeState>(positions, t+1);
            ...
            for (int j : problem->getGraph()->getNeighbors(positions[agentToAssign])) {
                ...
                newpositions[agentToAssign] = j;
                if (notAlreadyOccupiedPosition(j, positions, agentToAssign) && notAlreadyOccupiedEdge(j, positions, agentToAssign, prePositions) && problem->okForConstraints(agentToAssign, positions[agentToAssign], j, t+1)) {
                    recursiveAssignAMoveToAnAgent(agentToAssign+1, successors, cost+costMovement, newpositions, prePositions, t, violations+problem->numberOfViolations(agentToAssign, positions[agentToAssign], j, t+1));
                }
            }
            // Wait
            if (notAlreadyOccupiedPosition(positions[agentToAssign], positions, agentToAssign) && problem->okForConstraints(agentToAssign, positions[agentToAssign], t+1)) {
                recursiveAssignAMoveToAnAgent(agentToAssign+1, successors, cost+costWait, positions, prePositions, t, violations+problem->numberOfViolations(agentToAssign, positions[agentToAssign], t+1));
            }
        }
    } else { // obj_function=="SumOfCosts"
        if (agentToAssign==problem->getNumberOfAgents()-1){
            if (not (std::find(cannotMove.begin(), cannotMove.end(), agentToAssign) != cannotMove.end())){ // agentToAssign is allowed to move
                ...
                for (int j : problem->getGraph()->getNeighbors(positions[agentToAssign])) {
                    ...
                    newpositions[agentToAssign] = j;
                    if (notAlreadyOccupiedPosition(j, positions, agentToAssign) && notAlreadyOccupiedEdge(j, positions, agentToAssign, prePositions) && problem->okForConstraints(agentToAssign, positions[agentToAssign], j, t+1)) {
                        auto successor = std::make_shared<StandardMultiAgentSpaceTimeState>(newpositions, t+1, cannotMove);
                        successors->emplace_back(successor, cost+costMovement, violations+problem->numberOfViolations(agentToAssign, positions[agentToAssign], j, t+1));
                    }
                }
            ...
            if (not (std::find(cannotMove.begin(), cannotMove.end(), agentToAssign) != cannotMove.end())){ // agentToAssign is allowed to move
                ...
                for (int j : problem->getGraph()->getNeighbors(positions[agentToAssign])) {
                    ...
                    newpositions[agentToAssign] = j;
                    if (notAlreadyOccupiedPosition(j, positions, agentToAssign) && notAlreadyOccupiedEdge(j, positions, agentToAssign, prePositions) && problem->okForConstraints(agentToAssign, positions[agentToAssign], j, t+1)) {
                        recursiveAssignAMoveToAnAgent(agentToAssign+1, successors, cost+costMovement, newpositions, prePositions, t, violations+problem->numberOfViolations(agentToAssign, positions[agentToAssign], j, t+1), cannotMove);
                    }
                }
...
std::vector<std::tuple<std::shared_ptr<StandardMultiAgentSpaceTimeState>, int, int>> StandardMultiAgentAStarProblemWithConstraints::getSuccessors(std::shared_ptr<StandardMultiAgentSpaceTimeState> state) const {
    std::vector<std::tuple<std::shared_ptr<StandardMultiAgentSpaceTimeState>, int, int>> successors;
    auto positions = state->getPositions();
...
std::unordered_map<int, std::vector<int>> StandardMultiAgentAStarProblemWithConstraints::getPositions(std::vector<std::shared_ptr<StandardMultiAgentSpaceTimeState>> states) const {
...
std::shared_ptr<MultiAgentProblem> StandardMultiAgentAStarProblemWithConstraints::getProblem() {
    return problem;
}
