...
SingleAgentAStarProblem::SingleAgentAStarProblem(
    const std::shared_ptr<SingleAgentProblem> &problem)
    : AStarProblem<SingleAgentState>(), problem(problem) {}

std::shared_ptr<SingleAgentState>
SingleAgentAStarProblem::getStartState() const {
  return std::make_shared<SingleAgentState>(problem->getStart());
}

std::shared_ptr<SingleAgentState>
SingleAgentAStarProblem::getGoalState() const {
  return std::make_shared<SingleAgentState>(problem->getTarget());
}

bool SingleAgentAStarProblem::isGoalState(
    std::shared_ptr<SingleAgentState> state) const {
  return state->getPosition() == problem->getTarget();
}

std::vector<std::tuple<std::shared_ptr<SingleAgentState>, int, int>>
SingleAgentAStarProblem::getSuccessors(
    std::shared_ptr<SingleAgentState> state) const {
  std::vector<std::tuple<std::shared_ptr<SingleAgentState>, int, int>>
      successors;

  for (int newPosition :
       problem->getGraph()->getNeighbors(state->getPosition())) {
...
std::unordered_map<int, std::vector<int>> SingleAgentAStarProblem::getPositions(
    std::vector<std::shared_ptr<SingleAgentState>> states) const {
