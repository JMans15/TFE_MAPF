...
StandardMultiAgentAStarProblem::StandardMultiAgentAStarProblem(std::shared_ptr<MultiAgentProblem>  problem)
        : AStarProblem<StandardMultiAgentState>()
        , problem(std::move(problem))
{}

std::shared_ptr<StandardMultiAgentState> StandardMultiAgentAStarProblem::getStartState() const {
    return std::make_shared<StandardMultiAgentState>(problem->getStarts());
}

bool StandardMultiAgentAStarProblem::isGoalState(std::shared_ptr<StandardMultiAgentState> state) const {
...
bool StandardMultiAgentAStarProblem::notAlreadyOccupiedPosition(int position, std::vector<int> &positions, int agentToAssign) const {
...
bool StandardMultiAgentAStarProblem::notAlreadyOccupiedEdge(int position, const std::vector<int> &positions, int agentToAssign, const std::vector<int> &prePositions) const {
    for (int i = 0; i < agentToAssign; i++) {
...
void StandardMultiAgentAStarProblem::recursiveAssignAMoveToAnAgent(int agentToAssign, std::vector<std::tuple<std::shared_ptr<StandardMultiAgentState>, int, int>>* successors, int cost, std::vector<int> positions, const std::vector<int>& prePositions, std::vector<u_int8_t> cannotMove) const {
    int costMovement;
    int costWait;
        ...
        if (agentToAssign==problem->getNumberOfAgents()-1){
            ...
            for (int j : problem->getGraph()->getNeighbors(positions[agentToAssign])) {
                ...
                if (notAlreadyOccupiedPosition(j, positions, agentToAssign) && notAlreadyOccupiedEdge(j, positions, agentToAssign, prePositions)) {
                    auto successor = std::make_shared<StandardMultiAgentState>(newpositions);
            ...
            for (int j : problem->getGraph()->getNeighbors(positions[agentToAssign])) {
                ...
                newpositions[agentToAssign] = j;
                if (notAlreadyOccupiedPosition(j, positions, agentToAssign) && notAlreadyOccupiedEdge(j, positions, agentToAssign, prePositions)) {
                    recursiveAssignAMoveToAnAgent(agentToAssign+1, successors, cost+costMovement, newpositions, prePositions);
                }
            }
            // Wait
            if (notAlreadyOccupiedPosition(positions[agentToAssign], positions, agentToAssign)) {
                recursiveAssignAMoveToAnAgent(agentToAssign+1, successors, cost+costWait, positions, prePositions);
            }
        }
    } else { // obj_function=="SumOfCosts"
        if (agentToAssign==problem->getNumberOfAgents()-1){
            if (not cannotMove[agentToAssign]){ // agentToAssign is allowed to move
                ...
                for (int j : problem->getGraph()->getNeighbors(positions[agentToAssign])) {
                    ...
                    newpositions[agentToAssign] = j;
                    if (notAlreadyOccupiedPosition(j, positions, agentToAssign) && notAlreadyOccupiedEdge(j, positions, agentToAssign, prePositions)) {
                        auto successor = std::make_shared<StandardMultiAgentState>(newpositions, cannotMove);
                        successors->emplace_back(successor, cost+costMovement, 0);
                    }
                }
                ...
                if (positions[agentToAssign]!=problem->getTargets()[agentToAssign]) { // agentToAssign not at his target position
                    costWait = 1;
                    if (notAlreadyOccupiedPosition(positions[agentToAssign], positions, agentToAssign)) {
                        auto successor = std::make_shared<StandardMultiAgentState>(positions, cannotMove);
                        successors->emplace_back(successor, cost+costWait, 0);
                    }
            ...
            if (not cannotMove[agentToAssign]){ // agentToAssign is allowed to move
                ...
                for (int j : problem->getGraph()->getNeighbors(positions[agentToAssign])) {
                    ...
                    newpositions[agentToAssign] = j;
                    if (notAlreadyOccupiedPosition(j, positions, agentToAssign) && notAlreadyOccupiedEdge(j, positions, agentToAssign, prePositions)) {
                        recursiveAssignAMoveToAnAgent(agentToAssign+1, successors, cost+costMovement, newpositions, prePositions, cannotMove);
                    }
                }
                ...
                if (positions[agentToAssign]!=problem->getTargets()[agentToAssign]) { // agentToAssign not at his target position
                    costWait = 1;
                    if (notAlreadyOccupiedPosition(positions[agentToAssign], positions, agentToAssign)) {
                        recursiveAssignAMoveToAnAgent(agentToAssign+1, successors, cost+costWait, positions, prePositions, cannotMove);
                    }
                } else { // agentToAssign is at his target position
                    // agentToAssign can still move in the future
                    costWait = 1;
...
std::vector<std::tuple<std::shared_ptr<StandardMultiAgentState>, int, int>> StandardMultiAgentAStarProblem::getSuccessors(std::shared_ptr<StandardMultiAgentState> state) const {
    std::vector<std::tuple<std::shared_ptr<StandardMultiAgentState>, int, int>> successors;
    auto positions = state->getPositions();

    recursiveAssignAMoveToAnAgent(0, &successors, 0, positions, positions, state->getCannotMove());

    return successors;
}

std::unordered_map<int, std::vector<int>> StandardMultiAgentAStarProblem::getPositions(std::vector<std::shared_ptr<StandardMultiAgentState>> states) const {
...
std::shared_ptr<MultiAgentProblem> StandardMultiAgentAStarProblem::getProblem() {
    return problem;
}
