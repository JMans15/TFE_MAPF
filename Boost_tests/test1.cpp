...
BOOST_AUTO_TEST_SUITE(globalTests)
    ...
    BOOST_AUTO_TEST_CASE(two_agents_vertex_constraint) {
        ...
        solution = AStar<MultiAgentProblem, MultiAgentState>(problem, Manhattan).solve();
        BOOST_REQUIRE_MESSAGE(solution->getFoundPath(), "Found a path");
    ...
    BOOST_AUTO_TEST_CASE(one_constrained_agent) {
        ...
        int start = 4;
        int target = 6;
        auto problem = std::make_shared<SingleAgentSpaceTimeProblem>(g, start, target, Makespan, 0, std::set<VertexConstraint>{VertexConstraint{0,5,1}});
        auto solution = AStar<SingleAgentSpaceTimeProblem, SingleAgentSpaceTimeState>(problem, Manhattan).solve();
        BOOST_REQUIRE_MESSAGE(solution->getFoundPath(), "Found a path");
        BOOST_CHECK_MESSAGE(solution->getMakespanCost() == 3, "MakespanCost = " << solution->getMakespanCost() << " tested against 3");

        problem = std::make_shared<SingleAgentSpaceTimeProblem>(g, start, target, Fuel, 0, std::set<VertexConstraint>{VertexConstraint{0,5,1}});
        solution = AStar<SingleAgentSpaceTimeProblem, SingleAgentSpaceTimeState>(problem, Manhattan).solve();
        BOOST_REQUIRE_MESSAGE(solution->getFoundPath(), "Found a path");
    ...
    BOOST_AUTO_TEST_CASE(single_spacetime) {
        ...
        auto problem = std::make_shared<SingleAgentSpaceTimeProblem>(g, start, target, Fuel);
        auto solution1 = AStar<SingleAgentSpaceTimeProblem, SingleAgentSpaceTimeState>(problem, OptimalDistance).solve();
        BOOST_CHECK_MESSAGE(solution1->getFuelCost() == 18, "FuelCost = " << solution1->getFuelCost() << " tested against 18");

        problem = std::make_shared<SingleAgentSpaceTimeProblem>(g, start, target, Makespan);
        solution1 = AStar<SingleAgentSpaceTimeProblem, SingleAgentSpaceTimeState>(problem, Manhattan).solve();
