...
struct HardVertexConstraintEqual {
    bool operator()(const VertexConstraint& lhs, const VertexConstraint& rhs) const {
        return lhs.getAgent() == rhs.getAgent() && lhs.getPosition() == rhs.getPosition() && lhs.getTime() == rhs.getTime();
    }
};
...
struct HardEdgeConstraintEqual {
    bool operator()(const EdgeConstraint& lhs, const EdgeConstraint& rhs) const {
        return lhs.getAgent() == rhs.getAgent() && lhs.getPosition1() == rhs.getPosition1() && lhs.getPosition2() == rhs.getPosition2() && lhs.getTime() == rhs.getTime();
    }
};

using HardVertexConstraintsSet = std::unordered_set<VertexConstraint, HardVertexConstraintHash, HardVertexConstraintEqual>;
using HardEdgeConstraintsSet = std::unordered_set<EdgeConstraint, HardEdgeConstraintHash, HardEdgeConstraintEqual>;
