...
public:
  ...
  VertexConstraint(int agent, int position, int time, bool positive = false)
      : agent(agent), position(position), time(time), positive(positive) {}
  ...
  void print() const {
    if (positive) {
      std::cout << "Positive vertex constraint : agent " << agent
                << " has to be at position " << position << " at time " << time
                << "." << std::endl;
    } else {
      std::cout << "Vertex constraint : agent " << agent
                << " cannot be at position " << position << " at time " << time
                << "." << std::endl;
    }
  }
