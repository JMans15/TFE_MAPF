...
SingleAgentProblem::SingleAgentProblem(
    const std::shared_ptr<Graph> &graph, int start, int target,
    ObjectiveFunction m_objective, int agentId,
    ...
    : graph(graph), start(start), target(target), agentId(agentId),
      maxCost(maxCost), objective(m_objective),
      ...
      setOfSoftEdgeConstraints(setOfSoftEdgeConstraints), startTime(startTime) {
  if (m_objective == SumOfCosts) {
    objective = Makespan;
  }
  if (m_objective != Makespan && m_objective != Fuel &&
      m_objective != SumOfCosts) {
    objective = Makespan;
  }
  impossible = false;
...
SingleAgentProblem::SingleAgentProblem(const std::shared_ptr<Graph> &graph,
                                       int start, int target, int agentId,
                                       int maxCost)
    : SingleAgentProblem(graph, start, target, Makespan, agentId,
                         HardVertexConstraintsSet(), HardEdgeConstraintsSet(),
                         maxCost, SoftVertexConstraintsMultiSet(),
                         SoftEdgeConstraintsMultiSet(), 0) {}

bool SingleAgentProblem::okForConstraints(int position, int newPosition,
                                          int time) const {
  if (setOfHardVertexConstraints.find({agentId, newPosition, time}) ==
      setOfHardVertexConstraints.end()) {
    return setOfHardEdgeConstraints.find(
               {agentId, position, newPosition, time}) ==
...
bool SingleAgentProblem::okForConstraints(int newPosition, int time) const {
  return setOfHardVertexConstraints.find({agentId, newPosition, time}) ==
         setOfHardVertexConstraints.end();
}

int SingleAgentProblem::numberOfViolations(int position, int newPosition,
                                           int time) const {
  int count = 0;
...
int SingleAgentProblem::getStart() const { return start; }

int SingleAgentProblem::getTarget() const { return target; }

ObjectiveFunction SingleAgentProblem::getObjFunction() { return objective; }

int SingleAgentProblem::getAgentId() const { return agentId; }

bool SingleAgentProblem::isImpossible() const { return impossible; }

std::shared_ptr<Graph> SingleAgentProblem::getGraph() const { return graph; }

int SingleAgentProblem::getNumberOfAgents() const { return 1; }

int SingleAgentProblem::getMaxCost() const { return maxCost; }

int SingleAgentProblem::getStartTime() const { return startTime; }
...
SingleAgentProblem::getSetOfSoftEdgeConstraints() const {
  return setOfSoftEdgeConstraints;
}

SoftVertexConstraintsMultiSet
SingleAgentProblem::getSetOfSoftVertexConstraints() const {
...
SingleAgentProblem::getSetOfHardVertexConstraints() const {
