...
public:
    ...
    MultiAgentProblem(const std::shared_ptr<Graph>& graph, std::vector<int> starts, std::vector<int> targets,
                      ObjectiveFunction objective, const std::vector<int>& agentIds = std::vector<int>(),
...
private:
    ...
    // starts[index] is the start position of the agent at index
    std::vector<int> starts;

    // target is a list of length numberOfAgents with the target position of each agent
    // targets[index] is the target position of the agent at index
    ...
    // - SumOfCosts : The sum of the time steps required for every agent to reach its goal and remain at the goal position without leaving it again
