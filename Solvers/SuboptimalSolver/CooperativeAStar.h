...
// where we update a reservation table with the several (position, timestep)
// points of the planned paths These (position, timestep) points will be avoided
// during the searches of the remaining agents
//
// typeOfHeuristic is the heuristic for the single agent A* searches : Manhattan
// (Cooperative A*) or OptimalDistance (Hierarchical Cooperative A* with a
// Reverse Resumable A*)
//
// The search has 2 drawbacks here:
// - Agents don't cooperate once they have a planned path (for example, problem
// if narrow corridor)
//   Ideally, agent should allow to move of its target to allow others to pass.
// - It depends on the order of the agents (see prioritized planning Latombe
// 1991) Could be improved with WINDOWED Hierarchical Cooperative A*)
class CooperativeAStar {
public:
  CooperativeAStar(std::shared_ptr<MultiAgentProblem> problem,
  ...
  std::shared_ptr<Solution> solve() {
    ...
    for (int a = 0; a < problem->getNumberOfAgents(); a++) {
      ...
      auto prob = std::make_shared<SingleAgentProblem>(
          problem->getGraph(), problem->getStarts()[a],
          problem->getTargets()[a], problem->getObjFunction(),
          problem->getAgentIds()[a], verticesReservationTable,
          edgesReservationTable, maxCost,
      ...
      if (solution->getFoundPath()) {
        ...
        numberOfTimesteps =
            std::max(numberOfTimesteps, solution->getNumberOfTimesteps());
  ...
  solve(std::shared_ptr<MultiAgentProblem> m_problem) {
