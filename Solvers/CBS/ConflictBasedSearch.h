...
// designed to ensure that expanding a CT node N creates a disjoint partition of
// the space of solutions that satisfy the constraints of N. That is, every
...
protected:
  ...
  int numberOfVisitedNodes; // number of nodes for which we tested if the set of
                            // conflicts is empty or not
  ...
  // Returns a tuple {fullSetOfVertexConstraints, fullSetOfEdgeConstraints,
  // fullCosts, fullSolutions}
  // - fullSetOfVertexConstraints and fullSetOfEdgeConstraints are the sets of
  // constraints from the root node to this node
  // - fullCosts is a map from the id of an agent to the cost of the latest path
  // of this agent
  // - fullSolutions is a map from the id of an agent to the latest path of this
  // agent
  ...
  // - setOfPositiveConstraints is the set of positive constraints from the root
  // node to this node
  std::tuple<HardVertexConstraintsSet, HardEdgeConstraintsSet,
             std::unordered_map<int, int>, std::unordered_map<int, vector<int>>,
  ...
  // Returns the solution in node with the right format (Solution class and same
  // size for all paths)
  ...
  // Returns true if agentId can be at position between t1 and t2 according to
  // the setOfVertexConstraints
  static bool
