...
ConflictBasedSearch::ConflictBasedSearch(
    std::shared_ptr<MultiAgentProblem> problem, TypeOfHeuristic typeOfHeuristic,
    bool CAT, bool disjointSplitting)
    : problem(std::move(problem)), typeOfHeuristic(typeOfHeuristic),
      numberOfVisitedNodes(0), CAT(CAT), disjointSplitting(disjointSplitting) {}

ConflictBasedSearch::ConflictBasedSearch(TypeOfHeuristic typeOfHeuristic,
                                         bool CAT, bool disjointSplitting)
    : problem(nullptr), typeOfHeuristic(typeOfHeuristic),
      numberOfVisitedNodes(0), CAT(CAT), disjointSplitting(disjointSplitting) {}

std::tuple<std::unordered_map<int, std::vector<int>>, int,
           std::unordered_map<int, int>>
ConflictBasedSearch::planIndividualPaths() {
  ...
  for (int i = 0; i < problem->getNumberOfAgents(); i++) {
    ...
    auto prob = std::make_shared<SingleAgentProblem>(
        problem->getGraph(), problem->getStarts()[i], problem->getTargets()[i],
        problem->getObjFunction(), agentId,
...
std::set<AgentConflict> ConflictBasedSearch::calculateSetOfConflicts(
...
std::set<AgentConflict> ConflictBasedSearch::updateSetOfConflicts(
...
ConflictBasedSearch::retrieveSetsOfConstraintsAndCostsAndSolutions(
    std::shared_ptr<ConflictTreeNode> node) {
  HardVertexConstraintsSet fullSetOfVertexConstraints =
      problem->getSetOfHardVertexConstraints();
  HardEdgeConstraintsSet fullSetOfEdgeConstraints =
  ...
  return {fullSetOfVertexConstraints, fullSetOfEdgeConstraints, fullCosts,
          fullSolutions};
}
...
ConflictBasedSearch::
      ...
      if (node->getVertexConstraint()->isPositive()) {
        for (auto agentId : problem->getAgentIds()) {
          if (agentId != node->getVertexConstraint()->getAgent()) {
            fullSetOfVertexConstraints.insert(
                {agentId, node->getVertexConstraint()->getPosition(),
                 node->getVertexConstraint()->getTime()});
          }
        }
        ...
        if (node->getEdgeConstraint()->isPositive()) {
          for (auto agentId : problem->getAgentIds()) {
            if (agentId != node->getEdgeConstraint()->getAgent()) {
              fullSetOfEdgeConstraints.insert(
                  {agentId, node->getEdgeConstraint()->getPosition2(),
                   node->getEdgeConstraint()->getPosition1(),
                   node->getEdgeConstraint()->getTime()});
              fullSetOfEdgeConstraints.insert(
                  {agentId, node->getEdgeConstraint()->getPosition1(),
                   node->getEdgeConstraint()->getPosition2(),
                   node->getEdgeConstraint()->getTime()});
            }
          }
          setOfPositiveConstraints.insert(
              {node->getEdgeConstraint()->getAgent(),
               node->getEdgeConstraint()->getPosition1(),
               node->getEdgeConstraint()->getTime() - 1, true});
          setOfPositiveConstraints.insert(
              {node->getEdgeConstraint()->getAgent(),
               node->getEdgeConstraint()->getPosition2(),
               node->getEdgeConstraint()->getTime(), true});
        } else {
          fullSetOfEdgeConstraints.insert(*node->getEdgeConstraint());
        }
      }
  ...
  return {fullSetOfVertexConstraints, fullSetOfEdgeConstraints, fullCosts,
          fullSolutions, setOfPositiveConstraints};
}

std::unordered_map<int, vector<int>>
ConflictBasedSearch::retrieveSolutions(std::shared_ptr<ConflictTreeNode> node) {
  std::unordered_map<int, std::vector<int>> solutions;
...
std::shared_ptr<Solution> ConflictBasedSearch::combineSolutions(
    const std::shared_ptr<ConflictTreeNode> &node) {
  ...
  return std::make_shared<Solution>(node->getCost(), numberOfVisitedNodes,
                                    numberOfTimesteps, solutions,
...
std::shared_ptr<Solution> ConflictBasedSearch::solve() {
  ...
  if (cost <= problem->getMaxCost()) {
    fringe.insert(std::make_shared<ConflictTreeNode>(
        nullptr, nullptr, solution, costs, cost, nullptr, setOfConflicts));
  }
  ...
  while (!fringe.empty()) {
    ...
    auto [fullSetOfVertexConstraints, fullSetOfEdgeConstraints, fullCosts,
          fullSolutions] = retrieveSetsOfConstraintsAndCostsAndSolutions(node);
    ...
    for (int agentId : {agent1, agent2}) {
      ...
      if (conflict.isVertexConflict()) {
        ...
        successorVertexConstraint = std::make_shared<VertexConstraint>(
            agentId, conflict.getPosition1(), conflict.getTime());
        newFullSetOfVertexConstraints.insert(
            {agentId, conflict.getPosition1(), conflict.getTime()});
      } else { // edge conflict
        successorVertexConstraint = nullptr;
        if (agentId == agent1) {
          successorEdgeConstraint = std::make_shared<EdgeConstraint>(
              agentId, conflict.getPosition1(), conflict.getPosition2(),
              conflict.getTime());
          newFullSetOfEdgeConstraints.insert({agentId, conflict.getPosition1(),
                                              conflict.getPosition2(),
                                              conflict.getTime()});
        } else {
          successorEdgeConstraint = std::make_shared<EdgeConstraint>(
              agentId, conflict.getPosition2(), conflict.getPosition1(),
              conflict.getTime());
          newFullSetOfEdgeConstraints.insert({agentId, conflict.getPosition2(),
                                              conflict.getPosition1(),
                                              conflict.getTime()});
        }
      }
      auto prob = std::make_shared<SingleAgentProblem>(
          ...
          newFullSetOfVertexConstraints, newFullSetOfEdgeConstraints, INT_MAX,
          vertexConflictAvoidanceTable, edgeConflictAvoidanceTable);
      auto solution =
          AStar<SingleAgentAStarProblemWithConstraints,
                SingleAgentSpaceTimeState>(
      ...
      if (solution->getFoundPath()) {
        ...
        if (problem->getObjFunction() == Makespan) {
          ...
          successorCost =
              node->getCost() - fullCosts[agentId] + solution->getCost();
        }
        if (successorCost <= problem->getMaxCost()) {
          auto successorSetOfConflicts =
              updateSetOfConflicts(fullSolutions, node->getSetOfConflicts(),
                                   agentId, solution->getPathOfAgent(agentId));
          ...
          for (auto[agent, path] : fullSolutions){
              ...
              if (agent==agentId){
                  for (int t = 0 ; t < successorSolution[agentId].size(); t++){
                      std::cout << t << " : " << successorSolution[agentId][t]
          << std::endl;
                  }
              } else {
          ...
          fringe.insert(std::make_shared<ConflictTreeNode>(
              successorEdgeConstraint, successorVertexConstraint,
              successorSolution, successorCosts, successorCost, node,
              successorSetOfConflicts));
        }
      }
    }
  }
...
std::shared_ptr<Solution> ConflictBasedSearch::disjointSplittingSolve() {
  ...
  while (!fringe.empty()) {
    ...
    auto [fullSetOfVertexConstraints, fullSetOfEdgeConstraints, fullCosts,
          fullSolutions, setOfPositiveConstraints] =
        retrieveSetsOfConstraintsAndCostsAndSolutionsDisjointSplitting(node);
    ...
    int agentId = (std::rand() < RAND_MAX / 2) ? agent1 : agent2;
    ; // choose random agent
    // int agentId = agent1;
    ...
    auto next = setOfPositiveConstraints.lower_bound(
        VertexConstraint{agentId, 0, conflict.getTime()});
    if (next != setOfPositiveConstraints.end() and
        next->getAgent() == agentId) {
      t2 = next->getTime();
    ...
    auto prob = std::make_shared<SingleAgentProblem>(
        problem->getGraph(), position1, position2, problem->getObjFunction(),
        agentId, newFullSetOfVertexConstraints, newFullSetOfEdgeConstraints,
        INT_MAX, vertexConflictAvoidanceTable, edgeConflictAvoidanceTable, t1);
    auto solution =
        AStar<SingleAgentAStarProblemWithConstraints,
              SingleAgentSpaceTimeState>(
            std::make_shared<SingleAgentAStarProblemWithConstraints>(prob),
            typeOfHeuristic)
            .solve();
    if (solution->getFoundPath() and t1 + solution->getMakespanCost() <= t2 and
        okForHardConstraints(newFullSetOfVertexConstraints,
                             t1 + solution->getMakespanCost() + 1, t2 - 1,
                             position2, agentId)) {
      std::unordered_map<int, std::vector<int>> successorSolution;
      ...
      if (problem->getObjFunction() == Makespan) {
        ...
        successorCost =
            node->getCost() - fullCosts[agentId] + successorCosts[agentId];
      }
      if (successorCost <= problem->getMaxCost()) {
        auto successorSetOfConflicts =
            updateSetOfConflicts(fullSolutions, node->getSetOfConflicts(),
                                 agentId, successorSolution[agentId]);
        /*std::cout << "we add a node" << std::endl;
        for (auto constraint : newFullSetOfEdgeConstraints){
            constraint.print();
        }
    ...
    if (conflict.isVertexConflict()) {
      successorEdgeConstraint = nullptr;
      successorVertexConstraint = std::make_shared<VertexConstraint>(
          agentId, conflict.getPosition1(), conflict.getTime(), true);
      for (auto agent : problem->getAgentIds()) {
        if (agent != agentId) {
          newFullSetOfVertexConstraints.insert(
              {agent, conflict.getPosition1(), conflict.getTime()});
        }
      }
      setOfPositiveConstraints.insert(
          {agentId, conflict.getPosition1(), conflict.getTime(), true});
      ...
      if (agentId == agent1) {
        successorEdgeConstraint = std::make_shared<EdgeConstraint>(
            agentId, conflict.getPosition1(), conflict.getPosition2(),
            conflict.getTime(), true);
        for (auto agent : problem->getAgentIds()) {
          if (agent != agentId) {
            newFullSetOfEdgeConstraints.insert({agent, conflict.getPosition2(),
                                                conflict.getPosition1(),
                                                conflict.getTime()});
            newFullSetOfEdgeConstraints.insert({agent, conflict.getPosition1(),
                                                conflict.getPosition2(),
                                                conflict.getTime()});
          }
        }
        setOfPositiveConstraints.insert(
            {agentId, conflict.getPosition1(), conflict.getTime() - 1, true});
        setOfPositiveConstraints.insert(
            {agentId, conflict.getPosition2(), conflict.getTime(), true});
      } else {
        successorEdgeConstraint = std::make_shared<EdgeConstraint>(
            agentId, conflict.getPosition2(), conflict.getPosition1(),
            conflict.getTime(), true);
        ...
        setOfPositiveConstraints.insert(
            {agentId, conflict.getPosition2(), conflict.getTime() - 1, true});
        setOfPositiveConstraints.insert(
            {agentId, conflict.getPosition1(), conflict.getTime(), true});
      }
    }
    ...
    for (auto agentK : problem->getAgentIds()) {
      if (agentK != agentId) {
        if (conflict.isVertexConflict()) {
          if (fullSolutions[agentK][conflict.getTime()] ==
              conflict.getPosition1()) {
            agentsToReplan.insert(agentK);
            fullSolutions.erase(agentK);
          }
        } else {
          if (fullSolutions[agentK][conflict.getTime() - 1] ==
                  conflict.getPosition1() and
              fullSolutions[agentK][conflict.getTime()] ==
                  conflict.getPosition2()) {
            agentsToReplan.insert(agentK);
            fullSolutions.erase(agentK);
          } else if (fullSolutions[agentK][conflict.getTime() - 1] ==
                         conflict.getPosition2() and
                     fullSolutions[agentK][conflict.getTime()] ==
                         conflict.getPosition1()) {
            agentsToReplan.insert(agentK);
            fullSolutions.erase(agentK);
    ...
    for (auto agentK : agentsToReplan) {
      // Find the interval to replan between 2 landmarks
      auto next = setOfPositiveConstraints.lower_bound(
          VertexConstraint{agentK, 0, conflict.getTime()});
      if (next != setOfPositiveConstraints.end() and
          next->getAgent() == agentK) {
        t2 = next->getTime();
        position2 = next->getPosition();
      } else {
        t2 = INT_MAX;
      ...
      auto prob = std::make_shared<SingleAgentProblem>(
          problem->getGraph(), position1, position2, problem->getObjFunction(),
          agentK, newFullSetOfVertexConstraints, newFullSetOfEdgeConstraints,
          INT_MAX, vertexConflictAvoidanceTable, edgeConflictAvoidanceTable,
          t1);
      auto solution =
          AStar<SingleAgentAStarProblemWithConstraints,
                SingleAgentSpaceTimeState>(
              ...
              typeOfHeuristic)
              .solve();
      solutionOk = solution->getFoundPath() and
                   t1 + solution->getMakespanCost() <= t2 and
                   okForHardConstraints(newFullSetOfVertexConstraints,
                                        t1 + solution->getMakespanCost() + 1,
                                        t2 - 1, position2, agentK);
      if (solutionOk) {
        ...
        if (CAT) {
          ...
          for (int t = 1; t < fullSolutions[agentK].size(); t++) {
            edgeConflictAvoidanceTable.insert({agentK, fullSolutions[agentK][t],
                                               ...
                                               t});
            // edgeConflictAvoidanceTable.insert({agentK,
            // fullSolutions[agentK][t-1], fullSolutions[agentK][t], t});
          }
        }
      } else {
        break;
    ...
    if (successorCost <= problem->getMaxCost()) {
      ...
      fringe.insert(std::make_shared<ConflictTreeNode>(
          successorEdgeConstraint, successorVertexConstraint, successorSolution,
          successorCosts, successorCost, node, successorSetOfConflicts));
    }
  }
  LOG("No path has been found.");
  return std::make_shared<Solution>();
...
bool ConflictBasedSearch::okForHardConstraints(
    HardVertexConstraintsSet setOfVertexConstraints, int t1, int t2,
  ...
  if (t1 <= t2) {
    for (int t = t1; t <= t2; t++) {
      if (setOfVertexConstraints.find({agentId, position, t}) !=
          setOfVertexConstraints.end()) {
        return false;
      }
    }
  }
...
ConflictBasedSearch::solve(std::shared_ptr<MultiAgentProblem> m_problem) {
  problem = std::move(m_problem);
  numberOfVisitedNodes = 0;
  fringe = std::multiset<std::shared_ptr<ConflictTreeNode>,
                         ConflictTreeNodeComparator>();
  return solve();
}
