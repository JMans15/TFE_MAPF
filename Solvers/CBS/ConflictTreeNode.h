...
public:
  ConflictTreeNode(
      std::shared_ptr<EdgeConstraint> edgeConstraint,
      std::shared_ptr<VertexConstraint> vertexConstraint,
      std::unordered_map<int, std::vector<int>> solution,
      std::unordered_map<int, int> costs, int cost,
      std::shared_ptr<ConflictTreeNode> parent = nullptr,
      std::set<AgentConflict> setOfConflicts = std::set<AgentConflict>())
