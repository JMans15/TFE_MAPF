...
// - Enhanced version of ID (EID) : tries to resolve a conflict (by attempting
// to replan one group to avoid the plan of the other group) before merging the
// groups
//
...
private:
  ...
  bool replanGroupAAvoidingGroupB(const std::shared_ptr<Group> &groupA,
