...
ReverseResumableAStar::ReverseResumableAStar(
  ...
  heuristic = std::make_shared<ManhattanHeuristic<SingleAgentState>>(
      problem->getProblem()->getStart(),
      problem->getProblem()->getGraph()->getWidth());
  auto start = problem->getStartState();
  auto goal = problem->getGoalState();
  fringe.insert(std::make_shared<Node<SingleAgentState>>(
      goal, 0, heuristic->heuristicFunction(goal)));
  distance[goal] = 0;
}

int ReverseResumableAStar::resume(
    const std::shared_ptr<SingleAgentState> &target) {
  while (!fringe.empty()) {
...
int ReverseResumableAStar::optimalDistance(int position) {
  ...
  LOG("position is not yet explored, we'll continue the RRA* search to visit "
      "position")
